<template>
  <div class="bg-blue py-4 mx-auto">
    <div class="w-full h-screen bg-[url(~/assets/images/background.jpg)] bg-cover bg-center bg-fixed bg-no-repeat h-screen px-9">
      <img class="w-full px-16" src="data:image/png;base64,">
      <div class="flex flex-col">
          <div class="flex justify-between py-8 px-4 place-content-between">
              <div class="mr-10">
                  <p class="pt-4 text-4xl text-gray-600 font-semibold">陳乃嘉</p>
                  <p class="pt-4 text-xl text-gray-600 font-semibold">@{{ id }}</p>
              </div>
              <img class="h-24" src="https://opennccu.com/_nuxt/EmptyUser.26402e9e.png">
          </div>
          <div class="mt-10 px-1">
              <div class="space-y-6" action="#" method="POST">
                  <label class="block text-sm font-medium leading-6 text-gray-600">自我介紹</label>
                  <div class="mt-3">
                      <div class="w-full min-h-[5rem] block w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md break-words">
                          <p class="text-gray-700">
                              11111111111111111111
                          </p>
                      </div>
                  </div>
                  <div class="flex items-center justify-between">
                      <label for="password" class="block text-sm font-medium leading-6 text-gray-600">公開連結</label>
                  </div>
                  <div class="mt-2">
                      <div class="w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md">
                         <p class="text-gray-700">
                              ～～網址～～
                          </p>
                      </div>
                  </div>
                  <div class="mt-2">
                      <div id="introduction" class="block w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md">
                          <p class="text-gray-700">
                              ～～網址～～
                          </p>
                          </div>
                  </div>
              </div>
          </div>
        </div>
      </div>
  </div>
</template>


  
<script setup>
  const route = useRoute()
  const { id } = route.params
</script>