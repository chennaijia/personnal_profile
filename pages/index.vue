<template>
    <div class="bg-blue py-10 px-1 w-full max-w-screen-lg mx-auto">
        <div class="flex flex-col items-center">
            <div>
                <img class="w-full max-lg:max-w-xs px-16" src="data:image/png;base64,">
            </div>
            <div class="flex justify-betwee py-8 px-6">
                <div class="mr-10">
                    <p class="px-12 pt-4 text-4xl text-gray-600 font-semibold">陳乃嘉</p>
                    <p class="px-12 pt-4 text-xl text-gray-600 font-semibold">@naijia</p>
                </div>
                <img class="h-24" src="https://opennccu.com/_nuxt/EmptyUser.26402e9e.png">
            </div>
                <div class="mt-10">
                    <div class="space-y-6" action="#" method="POST">
                        <label class="block text-sm font-medium leading-6 text-gray-900">自我介紹</label>
                        <div class="mt-2">
                            <div id="introduction" class="block w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md">
                                <p class="text-gray-700">
                                    ～～自我介紹～～
                                </p>
                            </div>
                        </div>
                        <div class="flex items-center justify-between">
                            <label for="password" class="block text-sm font-medium leading-6 text-gray-900">公開連結</label>
                        </div>
                        <div class="mt-2">
                            <div id="introduction" class="block w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md">
                               <p class="text-gray-700">
                                    ～～網址～～
                                </p>
                            </div>
                        </div>
                        <div class="mt-2">
                            <div id="introduction" class="block w-full bg-white rounded-lg border border-gray-300 py-4 px-6 shadow-md">
                                <p class="text-gray-700">
                                    ～～網址～～
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
        </div>
    </div>
</template>

